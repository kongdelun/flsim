...
optimizer = tf.train.GradientDescentOptimizer(learning_rate=local_learning_rate).minimize(finalloss, var_list=None,
...
def local_run(user_x_train, user_y_train, experiment_iter):
    ...
    for epoch_i in range(local_iter):
        ...
        print("the training accuracy for epoch " + str(epoch_i + 1) + " loss: " + str(batch_loss) + " accuracy: " + str(
            batch_acc * 100))
...
(x_train, x_test, y_train, y_test) = data.generate_data(NUM_TRAIN_EXAMPLES_PER_USER, NUM_TEST_EXAMPLES_PER_USER, x_coll,
                                                        y_coll)
...
while True:
    ...
    for epoch_i in range(local_iter):
        ...
        print("the training accuracy for epoch " + str(outer_i * local_iter + epoch_i + 1) + " loss: " + str(
            batch_loss) + " accuracy: " + str(batch_acc * 100))
